...
# /////////////////////////////////////////////////////////////////////////////////////////////////////////////
# BLOCK: BUDYKO Hydrological model
# /////////////////////////////////////////////////////////////////////////////////////////////////////////////

# SOURCES:
# https://BUDYKO.walkerenvres.com/theory.html
...
f.BUDYKO <- function(parm) {
  ...
  P.BUDYKO <- BasinObs$P[Ind.Run]
  PE.BUDYKO <- BasinObs$E[Ind.Run]
  Qobs.BUDYKO <- BasinObs$Qmm[Ind.Run]
  
  # BUDYKOQest {hydrosystems} function is requested
  acbd.out <- BUDYKOQest(parm = parm[1:4],
                       P = P.BUDYKO,
                       PE = PE.BUDYKO,
                       S_ini = parm[5],
                       G_ini = parm[6],
  ...
  of.sse <- sum((Qobs.BUDYKO - acbd.out)^2,     na.rm = TRUE)
  of.nse <- (1-(hydroGOF::NSE(acbd.out, Qobs.BUDYKO)))
  of.kge <- (1-(hydroGOF::KGE(acbd.out, Qobs.BUDYKO)))
  of.kgenp <- (1-(hydroGOF::KGEnp(acbd.out, Qobs.BUDYKO)))
  of.lnse <- (abs(hydroGOF::NSE(log(acbd.out), log(Qobs.BUDYKO))))
  of.test01 <- 0.05*(of.nse) + 0.95*(of.nse)
  
  # sumQ <- sum(acbd.out)
  # df.sum <- data.frame(c(sum(acbd.out)/sum(Qobs.BUDYKO)))
...
f.BUDYKO(parm = parm.INI)

# Optimization by GenSA
CNT.BUDYKO <- GenSA(par = parm.INI,
                  fn = f.BUDYKO, 
                  lower = parm.LOW,
                  upper = parm.HIG,
                  control = list(maxit = 1000,
...
parm.exe.BUDYKO <- c(a = CNT.BUDYKO$par[1],   #1
                   b = CNT.BUDYKO$par[2],   #2
                   c = CNT.BUDYKO$par[3],   #3
                   d = CNT.BUDYKO$par[4],   #4
                   S_ini = CNT.BUDYKO$par[5],
                   G_ini = CNT.BUDYKO$par[6])

# Final KGE is requested
1 - f.BUDYKO(parm = parm.exe.BUDYKO)

# Optimal parameters are requested
print(parm.exe.BUDYKO)

# A final BUDYKO run is executed
acbd.out <- BUDYKOQest(parm = parm.exe.BUDYKO[1:4],
                     P = BasinObs$P[Ind.Run.Cal],
                     PE = BasinObs$E[Ind.Run.Cal],
                     S_ini = parm.exe.BUDYKO[5],
                     G_ini = parm.exe.BUDYKO[6],
                     print.all = FALSE)
...
# A data.frame container is defined
df.cont.BUDYKO.gensa.cal <- as.data.frame(t(c(hydroGOF::KGE(acbd.out, BasinObs$Qmm[Ind.Run.Cal]),
                                            hydroGOF::NSE(acbd.out, BasinObs$Qmm[Ind.Run.Cal]),
                                            hydroGOF::pbias(acbd.out, BasinObs$Qmm[Ind.Run.Cal]),
                                            hydroGOF::rPearson(acbd.out, BasinObs$Qmm[Ind.Run.Cal]),
                                            hydroGOF::d(acbd.out, BasinObs$Qmm[Ind.Run.Cal]),
...
# A final BUDYKO run is executed
acbd.out.val <- BUDYKOQest(parm = parm.exe.BUDYKO[1:4],
...
# A data.frame container is defined
df.cont.BUDYKO.gensa.val <- as.data.frame(t(c(hydroGOF::KGE(acbd.out.val, BasinObs$Qmm[Ind.Run.Val]),
                                            hydroGOF::NSE(acbd.out.val, BasinObs$Qmm[Ind.Run.Val]),
                                            hydroGOF::pbias(acbd.out.val, BasinObs$Qmm[Ind.Run.Val]),
                                            hydroGOF::rPearson(acbd.out.val, BasinObs$Qmm[Ind.Run.Val]),
                                            hydroGOF::d(acbd.out.val, BasinObs$Qmm[Ind.Run.Val]),
...
xls.object <- list("df.total.final.ofs.def" = df.total.final.ofs.def, #1
                   ...
                   "df.BUDYKO.final.par" = df.BUDYKO.final.par,           #3
...
# /////////////////////////////////////////////////////////////////////////////////////////////////////////////
# BLOCK: Local Sensitivity Analysis (SENSAN)
# /////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
#--------------------------------------------------------------------------------------------------------------
# SUBBLOCK: BUDYKO Hydrological model
# FME - Flexible Modelling Environment for Inverse Modelling
#--------------------------------------------------------------------------------------------------------------

# Initial fixed parameters are defined
parm.INI.BUDYKO <- c(a =  0.50, b = 350.00, c = 0.50,  d = 0.50, S_ini = 500.00, G_ini = 60.0)

# Sensitivity parameter ranges are defined
param.ranges.control.BUDYKO <- data.frame(min = c(parm.INI.BUDYKO [1]*0.90,
                                                parm.INI.BUDYKO [2]*0.90,
                                                parm.INI.BUDYKO [3]*0.90,
                                                parm.INI.BUDYKO [4]*0.90,
                                                parm.INI.BUDYKO [5]*0.90,
                                                parm.INI.BUDYKO [6]*0.90),
                                        max = c(parm.INI.BUDYKO [1]*1.10,
                                                parm.INI.BUDYKO [2]*1.10,
                                                parm.INI.BUDYKO [3]*1.10,
                                                parm.INI.BUDYKO [4]*1.10,
                                                parm.INI.BUDYKO [5]*1.10,
                                                parm.INI.BUDYKO [6]*1.10))

# data.frames variales are defined
rownames(param.ranges.control.BUDYKO) <- c("a", "b", "c", "d", "S_ini", "G_ini")

# BUDYKO hydrological model function is defined for sensitivity analysis
f.BUDYKO.sens <- function(parm) {
  
  # BUDYKO input objects are created
  P.BUDYKO <- BasinObs$P[Ind.Run.Cal]
  PE.BUDYKO <- BasinObs$E[Ind.Run.Cal]
  Qobs.BUDYKO <- BasinObs$Qmm[Ind.Run.Cal]
...
# BUDYKO hydrological model is run with control parameters
df.BUDYKO.out.control <- f.BUDYKO.sens(parm.INI.BUDYKO)

# Sensitivity Analysis via sensRange {FME} for parameter A
sR1.BUDYKO.A <- sensRange(func = f.BUDYKO.sens,
                 parms = parm.INI.BUDYKO, # these are the control parameters!!!
                 dist = "grid",
                 sensvar = c("kge"),
                 parRange = param.ranges.control.BUDYKO[c(1),], # parameter A
                 num = 100)

# Normalized Sensitivity Coefficient (NSC) data.frame is created
df.sR.BUDYKO.A.nsc <- sR1.BUDYKO.A

# sum_obs values are replaced by synthetic control values
df.sR.BUDYKO.A.nsc$kge_control <- as.numeric(df.BUDYKO.out.control$kge)

# A optimal value is added to data.frame
df.sR.BUDYKO.A.nsc$A_control <- parm.INI.BUDYKO[1]

# NSC SENSAN L2 norm sensitivities are calculated
df.sR.BUDYKO.A.nsc$L2 <- (abs(df.sR.BUDYKO.A.nsc$kge - df.sR.BUDYKO.A.nsc$kge_control)/
                          abs(df.sR.BUDYKO.A.nsc$a - df.sR.BUDYKO.A.nsc$A_control)) *
  (abs(df.sR.BUDYKO.A.nsc$A_control)/abs(df.sR.BUDYKO.A.nsc$kge_control))

# A simple plot is created
plot(df.sR.BUDYKO.A.nsc$a, df.sR.BUDYKO.A.nsc$L2)

# A simple boxplot is created
boxplot(df.sR.BUDYKO.A.nsc$L2)

# The median NSC value is requested
median(df.sR.BUDYKO.A.nsc$L2)

# Sensitivity Analysis via sensRange {FME} for parameter B
sR1.BUDYKO.B <- sensRange(func = f.BUDYKO.sens,
                        parms = parm.INI.BUDYKO, # these are the control parameters!!!
                        dist = "grid",
                        sensvar = c("kge"),
                        parRange = param.ranges.control.BUDYKO[c(2),], # parameter A
                        num = 100)

# Normalized Sensitivity Coefficient (NSC) data.frame is created
df.sR.BUDYKO.B.nsc <- sR1.BUDYKO.B

# sum_obs values are replaced by synthetic control values
df.sR.BUDYKO.B.nsc$kge_control <- as.numeric(df.BUDYKO.out.control$kge)

# A optimal value is added to data.frame
df.sR.BUDYKO.B.nsc$B_control <- parm.INI.BUDYKO[2]

# NSC SENSAN L2 norm sensitivities are calculated
df.sR.BUDYKO.B.nsc$L2 <- (abs(df.sR.BUDYKO.B.nsc$kge - df.sR.BUDYKO.B.nsc$kge_control)/
                          abs(df.sR.BUDYKO.B.nsc$b - df.sR.BUDYKO.B.nsc$B_control)) *
  (abs(df.sR.BUDYKO.B.nsc$B_control)/abs(df.sR.BUDYKO.B.nsc$kge_control))

# A simple plot is created
plot(df.sR.BUDYKO.B.nsc$b, df.sR.BUDYKO.B.nsc$L2)

# A simple boxplot is created
boxplot(df.sR.BUDYKO.B.nsc$L2)

# The median NSC value is requested
median(df.sR.BUDYKO.B.nsc$L2)

# Sensitivity Analysis via sensRange {FME} for parameter C
sR1.BUDYKO.C <- sensRange(func = f.BUDYKO.sens,
                        parms = parm.INI.BUDYKO, # these are the control parameters!!!
                        dist = "grid",
                        sensvar = c("kge"),
                        parRange = param.ranges.control.BUDYKO[c(3),], # parameter A
                        num = 100)

# Normalized Sensitivity Coefficient (NSC) data.frame is created
df.sR.BUDYKO.C.nsc <- sR1.BUDYKO.C

# sum_obs values are replaced by synthetic control values
df.sR.BUDYKO.C.nsc$kge_control <- as.numeric(df.BUDYKO.out.control$kge)

# A optimal value is added to data.frame
df.sR.BUDYKO.C.nsc$C_control <- parm.INI.BUDYKO[3]

# NSC SENSAN L2 norm sensitivities are calculated
df.sR.BUDYKO.C.nsc$L2 <- (abs(df.sR.BUDYKO.C.nsc$kge - df.sR.BUDYKO.C.nsc$kge_control)/
                          abs(df.sR.BUDYKO.C.nsc$c - df.sR.BUDYKO.C.nsc$C_control)) *
  (abs(df.sR.BUDYKO.C.nsc$C_control)/abs(df.sR.BUDYKO.C.nsc$kge_control))

# A simple plot is created
plot(df.sR.BUDYKO.C.nsc$c, df.sR.BUDYKO.C.nsc$L2)

# A simple boxplot is created
boxplot(df.sR.BUDYKO.C.nsc$L2)

# The median NSC value is requested
median(df.sR.BUDYKO.C.nsc$L2)

# Sensitivity Analysis via sensRange {FME} for parameter C
sR1.BUDYKO.D <- sensRange(func = f.BUDYKO.sens,
                        parms = parm.INI.BUDYKO, # these are the control parameters!!!
                        dist = "grid",
                        sensvar = c("kge"),
                        parRange = param.ranges.control.BUDYKO[c(4),], # parameter A
                        num = 100)

# Normalized Sensitivity Coefficient (NSC) data.frame is created
df.sR.BUDYKO.D.nsc <- sR1.BUDYKO.D

# sum_obs values are replaced by synthetic control values
df.sR.BUDYKO.D.nsc$kge_control <- as.numeric(df.BUDYKO.out.control$kge)

# A optimal value is added to data.frame
df.sR.BUDYKO.D.nsc$D_control <- parm.INI.BUDYKO[4]

# NSC SENSAN L2 norm sensitivities are calculated
df.sR.BUDYKO.D.nsc$L2 <- (abs(df.sR.BUDYKO.D.nsc$kge - df.sR.BUDYKO.D.nsc$kge_control)/
                          abs(df.sR.BUDYKO.D.nsc$d - df.sR.BUDYKO.D.nsc$D_control)) *
  (abs(df.sR.BUDYKO.D.nsc$D_control)/abs(df.sR.BUDYKO.D.nsc$kge_control))

# A simple plot is created
plot(df.sR.BUDYKO.D.nsc$d, df.sR.BUDYKO.D.nsc$L2)

# A simple boxplot is created
boxplot(df.sR.BUDYKO.D.nsc$L2)

# The median NSC value is requested
median(df.sR.BUDYKO.D.nsc$L2)
