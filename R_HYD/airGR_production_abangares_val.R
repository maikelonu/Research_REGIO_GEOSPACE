...
df.mumin <- subset(df.mumin, catchment == "abangares")
...
BasinObs <- read.table("airGR_abangares_2025.txt",header=T,sep="\t",quote="")
...
for (cc in 1:12) { # length = 12: months
  ...
  Ind_Run <- seq(which(format(BasinObs$DatesR, format = "%Y-%m")=="1971-05"),
                 which(format(BasinObs$DatesR, format = "%Y-%m")=="1984-08"))
  ...
  # GR2M run period is selected
  Ind_Run02 <- seq(which(format(BasinObs$DatesR, format = "%Y-%m")=="1984-08"),
                   which(format(BasinObs$DatesR, format = "%Y-%m")=="1990-12"))
