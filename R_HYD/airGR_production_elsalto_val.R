...
df.mumin <- subset(df.mumin, catchment == "elsalto")
...
BasinObs <- read.table("airGR_elsalto_2025.txt",header=T,sep="\t",quote="")

# ET0 is converted from mm/day to mm/month
BasinObs$E <- BasinObs$E*30
BasinObs$P <- (BasinObs$P)*0.80
...
factorC <- 1 # 0.0 to 1.00 proportional factor is created
...
for (cc in 1:12) { # length = 12: months
  ...
  Ind_Run <- seq(which(format(BasinObs$DatesR, format = "%Y-%m")=="1976-05"),
                 which(format(BasinObs$DatesR, format = "%Y-%m")=="1983-05"))
  ...
  # GR2M run period is selected
  Ind_Run02 <- seq(which(format(BasinObs$DatesR, format = "%Y-%m")=="1983-05"),
                   which(format(BasinObs$DatesR, format = "%Y-%m")=="1987-04"))
