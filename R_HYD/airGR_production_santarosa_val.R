...
df.mumin <- subset(df.mumin, catchment == "santarosa")
...
BasinObs <- read.table("airGR_santarosa_2025.txt",header=T,sep="\t",quote="")
...
for (cc in 1:12) { # length = 12: months
  ...
  Ind_Run <- seq(which(format(BasinObs$DatesR, format = "%Y-%m")=="1971-09"),
                 which(format(BasinObs$DatesR, format = "%Y-%m")=="1977-05"))
  ...
  # GR2M run period is selected
  Ind_Run02 <- seq(which(format(BasinObs$DatesR, format = "%Y-%m")=="1977-05"),
                   which(format(BasinObs$DatesR, format = "%Y-%m")=="1980-05"))
