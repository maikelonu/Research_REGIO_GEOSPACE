...
df.mumin <- subset(df.mumin, catchment == "tenorio")
...
BasinObs <- read.table("airGR_tenorio_2025.txt",header=T,sep="\t",quote="")

# ET0 is converted from mm/day to mm/month
BasinObs$E <- BasinObs$E*30
BasinObs$P <- BasinObs$P*0.90
...
for (cc in 1:12) { # length = 12: months
  ...
  Ind_Run <- seq(which(format(BasinObs$DatesR, format = "%Y-%m")=="1970-05"),
                 which(format(BasinObs$DatesR, format = "%Y-%m")=="1981-07"))
  ...
  # GR2M run period is selected
  Ind_Run02 <- seq(which(format(BasinObs$DatesR, format = "%Y-%m")=="1981-07"),
