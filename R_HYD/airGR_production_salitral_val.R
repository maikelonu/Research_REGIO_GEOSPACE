...
df.mumin <- subset(df.mumin, catchment == "salitral")
...
BasinObs <- read.table("airGR_salitral_2025.txt",header=T,sep="\t",quote="")

# ET0 is converted from mm/day to mm/month
BasinObs$E <- BasinObs$E*30
BasinObs$P <- BasinObs$P*0.85
...
for (cc in 1:12) { # length = 12: months
  ...
  Ind_Run02 <- seq(which(format(BasinObs$DatesR, format = "%Y-%m")=="1983-05"),
                   which(format(BasinObs$DatesR, format = "%Y-%m")=="1986-12"))
