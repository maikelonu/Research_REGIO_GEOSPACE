...
df.mumin <- subset(df.mumin, catchment == "lagarto")
...
BasinObs <- read.table("airGR_lagarto_2025.txt",header=T,sep="\t",quote="")
...
for (cc in 1:12) { # length = 12: months
  ...
  Ind_Run <- seq(which(format(BasinObs$DatesR, format = "%Y-%m")=="1970-05"),
                 which(format(BasinObs$DatesR, format = "%Y-%m")=="1981-12"))
  ...
  # GR2M run period is selected
  Ind_Run02 <- seq(which(format(BasinObs$DatesR, format = "%Y-%m")=="1981-12"),
