...
df.mumin <- subset(df.mumin, catchment == "canas")
...
BasinObs <- read.table("airGR_canas_2025.txt",header=T,sep="\t",quote="")
...
for (cc in 1:12) { # length = 12: months
  ...
  Ind_Run <- seq(which(format(BasinObs$DatesR, format = "%Y-%m")=="1966-01"),
                 which(format(BasinObs$DatesR, format = "%Y-%m")=="1980-01"))
  ...
  # GR2M run period is selected
  Ind_Run02 <- seq(which(format(BasinObs$DatesR, format = "%Y-%m")=="1982-01"),
